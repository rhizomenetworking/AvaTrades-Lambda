...
function makeAPIWallet(trade, bid) {
    ...
    let asset_ids = wallet.asset_ids.map(id => (0, utilities_1.stringFromAssetID)(id));
    return {
        "trade_id": trade.id,
        "address": (0, utilities_1.stringFromAddress)(wallet.chain, wallet.address),
        "asset_ids": asset_ids,
        "expiration": wallet.expiration.toString(),
        "chain": wallet.chain,
        "owner": (0, utilities_1.stringFromAddress)(wallet.chain, trade.proceeds_address)
    };
}
...
function makeAPIRoyalty(royalty) {
    return {
        "chain": royalty.chain,
        "asset_id": (0, utilities_1.stringFromAssetID)(royalty.asset_id),
        "divisor": royalty.divisor.toString(),
        "owner": (0, utilities_1.stringFromAddress)(royalty.chain, royalty.proceeds_address)
    };
}
